...
const bottom_correct_counter = document.querySelector("footer .correct_question");
...
var quizDatabase_Sorted = reference.child("user/").orderByChild("markDeducted");
...
function optionSelected(answer){
    ...
    let correcAns = questions[que_count].answer; //getting correct answer from array
    const allOptions = option_list.children.length; //getting all option items
    var timerLeft = parseInt(timeCount.innerHTML);

    if(userAns == correcAns ){ //if user selected option is equal to array's correct answer
        userScore += 1 ; //upgrading score value with 1
        userPoint += parseInt((timerLeft)*3.3333);
...
function queCounter(){
    // also the corrrect Qn Counter
    let totalCorrect = '<span><p>' + userScore + '</p> of <p>' + questions.length +'</p> Correct </span> <span> Score: <p>' +userPoint+ '</p></span>';
    bottom_correct_counter.innerHTML = totalCorrect;
